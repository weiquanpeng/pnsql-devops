<template>
  <div :class="sideNavCls">
    ...
    >
      ...
      <!-- 过滤后的 menu 数据传入 MenuContent -->
      <menu-content :nav-data="filteredMenu" />
      <template #operations>
        <span :class="versionCls"> {{ !collapsed ? 'PnSql Workflow' : '' }} {{ pgk.version }} </span>
      </template>
    </t-menu>
...
const filteredMenu = computed(() => {
  return menu.filter((item) => item.path !== '/404');
});
</script>
